...
from examples.train_so3krates_shnitsel import (
    get_full_system_list,
    load_system_data,
    merge_system_data_set,
)
...
def evaluate():
    ...
    if pn.force in targets:
        if pn.stress in targets:
            _test_obs_fn = jax.jit(
                jax.vmap(get_energy_force_stress_fn(test_net), in_axes=(None, 0))
            )
        else:
            _test_obs_fn = jax.jit(
                jax.vmap(get_obs_and_force_fn(test_net), in_axes=(None, 0))
            )
    else:
    ...
    def shift(_k, _v, _z):
        shifts = np.array(scales[prop_keys_inv[_k]]["per_atom_shift"], np.float64)[
            _z.astype(int)
        ].sum(
            axis=-1
        )  # shape: (B)
    ...
    def test_obs_fn(params, inputs):
        ...
        nn_out = jax.tree_util.tree_map(
            lambda _x: np.array(_x, dtype=np.float64), _test_obs_fn(params, inputs)
        )
    ...
    if evaluate_on == "train":
        ...
        raise ValueError(
            f"`--on {evaluate_on}` is invalid. Use one of `train`, `valid`, `test`."
        )

    loaded_data_arrays = None
    ...
    if apply_to is None:
        ...
        loaded_data_arrays = dataset_arrays
    else:
        raise NotImplementedError(
            "No support for non-default data files implemented yet"
        )
    ...
    if apply_to is None and from_split is not None:
        print(
            "Loading evaluation points according to the saved {} split in {} from {}.".format(
                evaluate_on, os.path.join(ckpt_dir, "splits.json"), coach.data_path
            )
        )
        ...
        test_data = loaded_data_arrays

        # TODO: FIXME: Deal with potential conversion
        ...
        test_data_set.load_split(
            file=os.path.join(ckpt_dir, "splits.json"),
            n_train=n_train,
            n_valid=n_valid,
            n_test=n_test,
            r_cut=r_cut,
        ...
        print(
            "Loading evaluation points according to the saved {} split in {} from {}.".format(
                evaluate_on, os.path.join(ckpt_dir, "splits.json"), apply_to
            )
        )
        ...
        msg = (
            "One should not end up here. This is likely due to a bug in the mlff package. Please report to "
            "https://github.com/thorben-frank/mlff"
        )
        raise RuntimeError(msg)

    test_data_tuple = DataTuple(
        inputs=coach.inputs, targets=targets, prop_keys=prop_keys
    )

    test_input, test_obs = test_data_tuple(d_test)

    test_metrics, test_obs_pred = evaluate_model(
        params=test_params,
        obs_fn=test_obs_fn,
        data=(test_input, test_obs),
        batch_size=batch_size,
        metric_fn={
            "mae": partial(mae_metric, pad_value=0),
