...
class DataSet:
    def __init__(self, prop_keys: Dict[str, str], data: Dict[str, np.ndarray]):
    ...
    def _correct_shapes(self):
        ...
        try:
            q_data[self.prop_keys[pn.energy]] = q_data[
                self.prop_keys[pn.energy]
            ].reshape(-1, 1)
        except KeyError:
            pass
        ...
        def reshape(y):
            if len(y.shape) <= 1:
                logging.warning(
                    "A 1D object has been extended with a new dimension as its first axis. Please provide vector-valued constants across all frames already as a 2d-array"
                )
        ...
        max_key = max(
            jax.tree_util.tree_map(lambda y: len(y), q_data),
        ...
        return {
            k: repeat(name=k, y=v, repeats=n_data) for (k, v) in q_data.items()
        }, n_data
    ...
    def average_number_of_neighbors(self) -> float:
        ...
        neighs = jax.vmap(p_segment_sum)(
            np.ones_like(idx_i), segment_ids=idx_i
        )  # shape: (n)
        return neighs.mean().item()

    def number_of_atoms(self) -> int:
        _n_atoms = (self.data_split["train"][self.prop_keys[pn.atomic_type]] != -1).sum(
            -1
        )  # shape: (D)
        if _n_atoms.std() != 0:
            logging.warning(
                "Dataset contains structures with different structure sizes. Number of atoms is thus not"
                "well defined."
            )
        ...
        if r_cut is not None:
            ...
            if training:
                ...
                if node_msk_needed | node_mask_present:
                    n_msk_dat = np.concatenate(
                        [d["train"][n_msk_key], d["valid"][n_msk_key]]
                    )
                else:
                    n_msk_dat = np.ones_like(z_dat).astype(bool)

            else:
                R_dat = np.concatenate(
                    [d["train"][R_key], d["valid"][R_key], d["test"][R_key]]
                )
                z_dat = np.concatenate(
                    [d["train"][z_key], d["valid"][z_key], d["test"][z_key]]
                )
            ...
            if mic:
                ...
                unit_cell_dat = np.concatenate(
                    [d["train"][uc_key], d["valid"][uc_key], d["test"][uc_key]]
                )
                pbc_dat = np.concatenate(
                ...
                if 0.5 * min(cell_lengths) <= r_cut:
                    ...
                    #                           f'reduce r_cut.')
                    logging.warning(
                        f"r_cut > 0.5*min(cell_lengths) detected! r_cut={r_cut} and "
            ...
            if mic:
                c_off_train, c_off_valid, c_off_test = np.split(
                    neigh_idxs["shifts"],
                ...
                d_idx = {
                    "train": {
                        "idx_i": idx_i_train,
                        "idx_j": idx_j_train,
                        "cell_offset": c_off_train,
                    },
                    "valid": {
                        "idx_i": idx_i_valid,
                        "idx_j": idx_j_valid,
                        "cell_offset": c_off_valid,
                    },
                    "test": {
                        "idx_i": idx_i_test,
                        "idx_j": idx_j_test,
                        "cell_offset": c_off_test,
                    },
                }
            else:
                d_idx = {
                    "train": {"idx_i": idx_i_train, "idx_j": idx_j_train},
                    "valid": {"idx_i": idx_i_valid, "idx_j": idx_j_valid},
                    "test": {"idx_i": idx_i_test, "idx_j": idx_j_test},
                }

            _d = {
                k: merge_dicts(v, d_idx[k])
                for k, v in d.items()
            ...
            if node_mask_present:
                ...
                n_msk_train, n_msk_valid, n_msk_test = np.split(
                    n_msk_dat, indices_or_sections=[n_train, n_train + n_valid]
                )

                d_n_msk = {
                    "train": {"node_mask": n_msk_train},
                    "valid": {"node_mask": n_msk_valid},
                    "test": {"node_mask": n_msk_test},
                }

                _d_n_msk = {
                    k: merge_dicts(v, d_n_msk[k])
                    for k, v in d.items()
                    if k in ["train", "valid", "test"]
                }
    ...
    def random_split(
        self, n_train, n_valid, n_test, r_cut=None, mic=None, seed=0, training=True
    ):

        idx_train, idx_valid, idx_test = self.generate_split_indices(
            self.data, n_train=n_train, n_valid=n_valid, n_test=n_test, seed=seed
        )

        self.index_split(
            data_idx_train=idx_train,
    ...
    def generate_split_indices(
        data, n_train, n_valid, n_test=None, seed=0, draw_strat=None
    ):
        set_seeds(seed)
        _k = list(data.keys())[0]
        n_data = len(data[_k])
        ...
        if draw_strat:
            raise NotImplementedError(
                "Stratified data set sampling is not implemented."
            )
        ...
        idx_test = idx_test[test_perm][
            :n_test
        ]  # array[:None] returns all elements of array
    ...
    def divide_x_by_std_y(self, x, y):
        if x in self.track_divide_x_by_std_y:
            logging.warning(
                f"You already called `divide_x_by_std_y` for `x={x}`. It is not divided again."
            )
        else:
    ...
    def shift_x_by_type_hand(self, x, shifts: Dict[int, float]):
        if x in self.track_shift_x_by_type:
            logging.warning(
                f"You already called `shift_x_by_type` for `x={x}`. It is not shifted again."
            )
        else:
            x_key = self.prop_keys[x]
            z_key = self.prop_keys[pn.atomic_type]
            ...
            self.data_split["train"][x_key] = apply_shifts(
                self.data_split["train"][x_key].reshape(-1),
                self.data_split["train"][z_key],
            ).reshape(self.data_split["train"][x_key].shape)

            self.data_split["valid"][x_key] = apply_shifts(
                self.data_split["valid"][x_key].reshape(-1),
                self.data_split["valid"][z_key],
            ).reshape(self.data_split["valid"][x_key].shape)
    ...
    def shift_x_by_type_lse(self, x):
        if x in self.track_shift_x_by_type:
            ...
            shifts, x_shift_lse = get_per_atom_shift(
                z=z, q=self.data_split["train"][x_key].reshape(-1), pad_value=0
            )
            ...
            self.data_split["train"][x_key] = x_shift_lse.reshape(
    ...
    def load_split(
        ...
        key_2_n = {
            ...
            "data_idx_test": n_test,
        }

        valid_keys = list(key_2_n.keys())

        subset_split = {
            k: v[: key_2_n[k]] for (k, v) in split_idx.items() if k in valid_keys
        }
    ...
    def save_splits_to_file(self, path, filename):
        ...
        print(
            "Saved the data indices of the splits to {}".format(
