...
def train_step_fn(
    state: TrainState, batch: Dict, loss_fn: Callable
    ...
    """
    (loss, train_metrics), grads = jax.value_and_grad(loss_fn, has_aux=True)(
        state.params, batch
    )
...
def valid_epoch(
    state: TrainState, ds: DataTupleT, metric_fn: LossFn, bs: int
...
@partial(jax.jit, static_argnums=2)
def valid_step_fn(
    state: TrainState, batch: DataTupleT, metric_fn: Callable
...
def run_training(
    ...
    train_bs: int,
    valid_bs: int,
    metric_fn: LossFn | None = None,
    epochs: int = 100,
    ckpt_dir: str | None = None,
    ckpt_manager_options: dict | None = None,
    eval_every_t: int | None = None,
    log_every_t: int | None = None,
    restart_by_nan: bool = True,
    stop_lr_fn: Callable[[float], bool] | None = None,
    stop_metric_fn: Callable[[Dict[str, float]], bool] | None = None,
    ...
    with ocp.CheckpointManager(
        ...
        for i in range(1, int(steps_per_epoch * epochs) + 1):
            ...
            train_batch = jax.tree_util.tree_map(
                lambda y: y[perms[step_in_epoch], ...], train_ds
            )
            if loss_fn_input is not None:
                loss_input_batch = jax.tree_util.tree_map(
                    lambda y: y[perms[step_in_epoch], ...], loss_fn_input
                )
                train_batch = train_batch + loss_input_batch
            train_start = time.time()
            state, train_batch_metrics, grads = train_step_fn(
                state=state, batch=train_batch, loss_fn=loss_fn
            )
            train_end = time.time()
            ...
            if (
                np.isnan(train_batch_metrics_np["loss"])
                or np.isinf(train_batch_metrics_np["loss"])
                or np.isnan(train_batch_metrics_np["gradients_norm"])
                or np.isinf(train_batch_metrics_np["gradients_norm"])
            ) and restart_by_nan:

                if np.isnan(train_batch_metrics_np["loss"]) or np.isinf(
                    train_batch_metrics_np["loss"]
                ):
                    logging.warning(
                        f"NaN detected during training in step {i} in the loss function value. Reload the "
                        "last checkpoint."
                    )

                if np.isnan(train_batch_metrics_np["gradients_norm"]) or np.isinf(
                    train_batch_metrics_np["gradients_norm"]
                ):
                    logging.warning(
                        f"NaN detected during training in step {i} in gradient values. Reload the "
                        "last checkpoint."
                    )

                grads_np = jax.tree_util.tree_map(
                    lambda x: x.tolist(), jax.device_get(grads)
                )
                save_dict(
                    ckpt_dir,
                    filename=f"gradients_nan_step_{i}.json",
                    data=unfreeze(grads_np),
                    exists_ok=True,
                )
            ...
            valid_start, valid_end, n_valid = (0.0, 0.0, 1.0)
            evaluate = i % eval_every_t == 0
            if evaluate:
                valid_start = time.time()
                valid_metrics, n_valid = valid_epoch(
                    state, valid_ds, metric_fn, bs=valid_bs
                )
                valid_end = time.time()
                ...
                if stop_metric_fn is not None:
                    if stop_metric_fn(valid_batch_metrics_np):
                        logging.info(
                            f"Stopping criteria for metrics has been met for step {i}."
                        )
                        exit()
            ...
            if i % log_every_t == 0:
                ...
                if use_wandb:
                    wandb.log(
                        {
                            "Training {}".format(k): v
                            for (k, v) in train_batch_metrics_np.items()
                        },
                        step=i,
                    )
                else:
                ...
                if stop_lr_fn is not None:
                    if stop_lr_fn(abs(lr)):
                        logging.info(
                            f"Stopping criteria for learning rate has been met for step {i}."
                        )
                        exit()
